...
export default function ProductPage() {
  ...
  const handleRelatedAddToCart = async (product: any) => {
    setRelatedLoading(prev => new Set(prev).add(product.id));
    try {
      const success = await addToCart({
        farmerId: "1",
                ...
                <Card key={p.id} className="cursor-pointer hover:shadow-lg transition-shadow">
                  ...
                  <CardContent className="p-3">
                    ...
                    <Button 
                      ...
                      disabled={relatedLoading.has(p.id)}
                    >
                      {relatedLoading.has(p.id) ? "Adding..." : "Add to Cart"}
                    </Button>
                  </CardContent>
                </Card>
              ))}
